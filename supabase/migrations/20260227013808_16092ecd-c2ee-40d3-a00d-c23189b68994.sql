...
-- Fix process_referral_commission to prevent recursion when called by add_points_with_cap
-- Uses a session-level flag (GUC) to prevent re-entry
...
BEGIN
  -- Recursion guard: check if we're already inside a commission calculation
  BEGIN
  ...
  IF v_in_commission THEN
    -- We're being triggered by our own add_points_with_cap call — skip to prevent infinite loop
    RETURN NEW;
  END IF;
  ...
  -- Set recursion guard before calling add_points_with_cap
