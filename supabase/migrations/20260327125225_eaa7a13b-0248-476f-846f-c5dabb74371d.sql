...
-- Update get_streak_multiplier to be weekly-based:
-- +10% per consecutive week (7 days), max 100% at 10 weeks (70 days)
-- Resets to 1.0 if streak breaks (handled by existing streak tracking)
...
BEGIN
  -- Every 7 consecutive days = +10% boost
  -- Max 10 weeks = +100% (2x multiplier)
  v_weeks := FLOOR(streak_days / 7);
  multiplier := 1.0 + LEAST(v_weeks * 0.1, 1.0);
  RETURN multiplier;
END;
$function$;

-- Update add_points_with_cap to also include referral miner_boost_percentage
-- This stacks: early_user_boost * streak_boost * (1 + referral_boost/100) * time_multiplier
...
BEGIN
  ...
  -- early_user_boost * time * streak * referral_boost
