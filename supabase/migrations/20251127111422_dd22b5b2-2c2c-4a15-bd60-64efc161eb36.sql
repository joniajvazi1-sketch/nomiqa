-- Create a function to send membership tier upgrade email notification
...
-- Create trigger to fire after membership tier is updated
DROP TRIGGER IF EXISTS on_membership_tier_upgrade ON public.user_spending;
CREATE TRIGGER on_membership_tier_upgrade
  AFTER UPDATE OF membership_tier ON public.user_spending
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_membership_tier_upgrade();